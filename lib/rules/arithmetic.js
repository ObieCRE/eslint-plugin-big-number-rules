...
function arithmeticEntry(context) {
  ...
  function SuggestReplacement(node) {
    ...
    const baseErrorMessage = `please convert '${original}' to us Big.js methods to avoid floating point math errors`;
...
function arithmeticExit(context) {
  return exitNode => {
    ...
    if (!immediateParentIsSummable(node)) {
      context.report({
        node,
        message: `please convert '${original}' to us Big.js methods to avoid floating point math errors`,
        fix: fixer => {
