...
const getSupportsSum = makeSettingGetter('supportsSum', false)
const getSupportsBitwise = makeSettingGetter('supportsBitwise', false)
...
function arithmeticEntry(context) {
  ...
  function SuggestReplacement(node) {
    ...
    const baseErrorMessage = `please convert '${original}' to use Big.js methods to avoid floating point math errors`;
    const errorMessage = baseErrorMessage + (original.match(/\+/) ? ' OR if this is for string concatenation you can use concat() or explicitly ignore this eslint error for this line.' : '')
...
function arithmeticExit(context) {
  return exitNode => {
    ...
    if (!immediateParentIsSummable(node)) {
      context.report({
        node,
        message: `please convert '${original}' to use Big.js methods to avoid floating point math errors`,
        fix: fixer => {
